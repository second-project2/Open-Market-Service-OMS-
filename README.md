...
<table>
  ...
  <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR8Br-BZrPih1vBxXY3CYfqiZ0SksqzH261o9Jw0OfN5iu2GWTowzCbZA-sTqzxaCaoy33U5p8XpiZ2T40pIXF8SqaDfgHlj8XSjr6kBQ&s=10" width="150px" style="object-fit: cover;" />
        ...
        <td align="center"><img src="<img src="https://github.com/gsbymsui83-commits/my-assets/blob/main/%EC%A6%9D%EB%AA%85.jpg?raw=true" width="150" height="150" style="object-fit: cover;></td>
    ...
    <tr>
        ...
        <td align="center"><a href="https://github.com/팀원아이디">@gsbymsui83-commits</a></td>
...
| **수 (Day 2)** | **UI 구조<br>& 퍼블리싱** | **전원** | - **전준성**: 구매자/판매자 회원가입 탭 UI, 입력 폼 레이아웃<br>- **강지연**: 로그인 폼 UI, 경고 메시지 영역 확보<br>- **강명주**: 메인 GNB/Footer 구현, 상품 목록 카드 레이아웃<br>- **김범규**: 상품 정보/이미지 배치, 옵션 선택 UI |
...
| **강명주** | **FE** | **메인 목록 및 GNB 시스템**<br>- 상품 목록 API 연동 및 데이터 파싱 최적화 (auto-fill 그리드 구현)<br>- 검색 엔진 고도화(URL 파라미터 기반 배너 제어 및 검색어 인코딩 처리)<br>- GNB/푸터 컴포넌트화 및 비동기 레이아웃 로드 로직 구현<br>- 공통 모달 모듈화<br>- 전방위 반응형 최적화(GNB 아이콘 개편 및 푸터 유동적 레이아웃 수정) |
...
- **해결 과정:**
  ...
  3. 가장 먼저 보이는 첫 번째 슬라이드 이미지에 <link rel="preload">와 fetchpriority="high" 속성을 부여했습니다.
  4. 배너 내부의 .banner-text 박스에 min-height를 설정하여 텍스트 박스의 높이가 변하며 화면이 아래로 밀리는 현상을 방지했습니다.
...
> 기술적 성장: 자바스크립트를 활용한 실무 구현이 처음이었기에 초기에는 막연한 두려움이 있었으나, 직면한 문제들을 단계별로 추적하고 해결하며 개발의 즐거움을 체감했습니다. 특히 이론 수업 시 혼란스러웠던 '변수 선언을 통한 상태 관리(State Management)'와 '함수 호출 및 매개변수 전달을 통한 실행 로직 제어'의 개념이, 실제 데이터가 화면에 렌더링되는 과정을 거치며 새롭게 입체적으로 이해되었습니다. 이 경험은 단순한 코드 작성을 넘어 프로그램의 내부 동작 원리를 더 깊이 탐구하고 싶게 만드는 강력한 동기부여가 되었습니다.
> 협업의 가치: 첫 팀 프로젝트를 통해 팀원 간의 상호 신뢰와 전문성 존중의 중요성을 배웠습니다. 그리고 코드 중복 여부를 사전에 조금 더 꼼꼼히 검토하고 로직을 공통화'하는 과정이 개발 효율성을 올릴 수 있겠다고 느꼈습니다. 서로의 작업 영역을 공유하고 유연하게 업무를 분담하는 커뮤니케이션 과정은 개인의 성장을 넘어 팀의 시너지를 만들어내는 소중한 경험이었습니다.
