...
* **상품 상세 및 가격 로직**: 수량 변경에 따른 실시간 총액 계산 시스템, 재고(Stock) 수량에 따른 구매 버튼 활성화/비활성화 처리.
...
| | | **강명주** | - 상품 목록 조회 API(GET) 연동 및 동적 데이터 렌더링<br>- 상품 검색 시스템 구축<br>- 상품리스트 클릭시 상세페이지 이동 로직 구현 |
...
| | | **강명주** | - 마이페이지 드롭다운 기능 구현<br>- 로그인 안내 모달 로직 단일화<br>- 업무 코드 중복 제거를 위한 헤더/푸터 분리작업 |
...
gantt
    ...
    인증 로직 및 예외 UX 구현(강지연)      :day3_2, 2026-01-15, 1d
    ...
    기능점검 & 정리 (강지연)   :day4_2, 2026-01-16, 1d
...
### 5 화면 설계 (Key Features)

|  메인 페이지 (Index) |  상품 상세 페이지 |
| :---: | :---: |
| <img src="https://github.com/user-attachments/assets/ccfc39d7-5606-43be-8548-fd4673d4e1d0" width="100%" /> | <img src="https://github.com/user-attachments/assets/d495d702-2ab5-4e18-8733-60478a68e140" width="100%" /> |
| **로그인 페이지** | **회원가입 페이지** |
| <img src="https://github.com/user-attachments/assets/3df62141-3abd-4a6b-9bd4-9b97c45ae299" width="100%" /> | <img src="https://github.com/user-attachments/assets/0cd439b8-13dd-44e8-a7c9-3ecca2bf4bf3" width="100%" /> |

<br>

|  404 Error 페이지 |
| :---: |
| <img src="https://github.com/user-attachments/assets/4ea90399-cf4f-4acc-bc8a-d4d65981931b" width="60%" /> |
...
- **해결 과정:**
  ...
  4. 
### 6.2 [성능] Lighthouse 지표 개선 (49점 → 88점 달성)
...
- **해결 과정**:
  1. **이미지 최적화**: 포맷을 **WebP**로 변환하고 용량을 압축하여 전송 속도를 대폭 향상했습니다.
  2. **레이아웃 안정화**: 이미지 컨테이너에 `aspect-ratio`를 부여하여 이미지 로딩 전 공간을 확보(CLS 개선)했습니다.
  3. **로딩 전략 수정**: 중요 리소스에 `preload`를 적용하고, 스크립트 태그에 `defer`를 부여해 렌더링 차단을 방지했습니다.
...
- **성과**: 
  - 성능 점수 **88점(Orange)** 달성 및 LCP 지표를 **4.1s**로 단축(약 97% 개선).
  - TBT(Total Blocking Time)를 970ms에서 60ms로 줄여 사용자 인터랙션 반응 속도 최적화.

### 6.3 [로직] 비동기 통신 에러 핸들링 (Async/Await)
...
### 6.4 [데이터] 상품 상세 API 데이터 구조 불일치 문제 (김범규)
- **상황:** 상품 상세 페이지 구현 중, 서버에서 데이터를 정상적으로 받아왔음에도 불구하고 화면에 가격과 판매자 정보가 표시되지 않는 문제가 발생했습니다.
- **원인:** 팀원이 API 명세서를 확인하는 과정에서 데이터 필드명을 혼동하여, price 대신 임의의 변수명(selling_price)으로 데이터를 호출한 커뮤니케이션 및 코드 작성 실수가 있었습니다.
- **해결 과정:**
  1. `console.log`로 실제 응답 구조를 확인했습니다.
  2. 렌더링 로직을 `price` 기준으로 수정했습니다.
  3. 값이 없을 경우를 대비한 예외 처리도 함께 추가했습니다.
- **성과:** 가격 정보가 정상 렌더링되고, 구조 변경에도 대응 가능한 안정성이 확보되었습니다.

### 6.5 [로직] 상품 수량 초과 입력 예외 처리 (김범규)
...
### 6.6 [로직] Enter 키 제출 처리 개선 (submit 이벤트 사용)

**고민**
초기에는 `type="submit"` 버튼에 click 이벤트를 등록하여 로그인 로직을 처리했습니다.  
그러나 Enter 키 입력 시에도 click 이벤트가 실행되었으나, 코드 리뷰 후 click 이벤트를 submit 이벤트로 변경했더니 form의 기본 submit 동작이 함께 실행되어 HTTP 요청이 발생하면서 의도치 않은 화면으로 이동하거나 페이지가 새로고침되는 문제가 발생했습니다.

**해결**
로그인 로직을 버튼의 click 이벤트에서 form의 submit 이벤트로 이동시키고, `event.preventDefault()`로 기본 submit 동작을 차단했습니다.  
이를 통해 버튼 클릭과 Enter 입력 모두 동일한 흐름으로 안정적으로 처리되도록 개선했습니다.


### 7. 개발하며 느낀점
**전준성 (Team Leader)**
> **"설계의 견고함으로 팀의 효율을 이끄는 리더"**
