...
### 5.1 [협업] 충돌 방지를 위한 MPA 구조 설계
- **배경:** 초기 기획 단계에서 4명의 팀원이 동시에 개발할 때 발생할 수 있는 `Merge Conflict`를 최소화하는 것이 목표였습니다.
- **전략:** SPA(Single Page) 대신 **MPA(Multi Page) 아키텍처**를 채택하여 `login.html`, `signup.html` 등 페이지별로 파일과 작업 영역을 물리적으로 분리했습니다.
- **결과:** 각 팀원의 작업 파일이 명확히 나뉘어 Git 병합 시 **코드 충돌 0건**을 기록했으며, 기능 구현에만 집중할 수 있는 효율적인 협업 환경을 조성했습니다.

### 5.2 [성능] Lighthouse 점수 48점 → 90점대 개선
...
- **성과:** 최적화 후 성능 점수 **90점(Green)** 달성 및 페이지 로딩 속도를 획기적으로 단축했습니다.
